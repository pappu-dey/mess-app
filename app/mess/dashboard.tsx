...
import { Download } from "lucide-react-native"; // Add to existing lucide imports
import { generateMemberStatsExcel, generateMemberStatsPDF } from "../../utils/exportUtils";
...
export default function Dashboard() {
  ...
  const [showMemberStatsExportMenu, setShowMemberStatsExportMenu] = useState(false);
  const [isExportingMemberStats, setIsExportingMemberStats] = useState(false);
  ...
  const handleMemberStatsExport = async (format: "pdf" | "excel") => {
    if (memberStats.length === 0) {
      Alert.alert("No Data", "No member statistics available to export");
      return;
    }

    try {
      setIsExportingMemberStats(true);
      setShowMemberStatsExportMenu(false);
      ...
      if (format === "pdf") {
        fileUri = await generateMemberStatsPDF(memberStats, monthStr);
        fileName = `Member_Statistics_${monthName}.pdf`;
      } else {
        fileUri = await generateMemberStatsExcel(memberStats, monthStr);
        fileName = `Member_Statistics_${monthName}.csv`;
      }

      // Use the saveFile utility to save permanently
      await saveFile(fileUri, fileName, format === "pdf" ? "pdf" : "excel");

    } catch (error) {
      console.error("Export error:", error);
      Alert.alert("Error", `Failed to generate ${format.toUpperCase()} report`);
    } finally {
      setIsExportingMemberStats(false);
    }
  };
      ...
      >
        ...
        >
          ...
          <View style={styles.membersCard}>
            {/* Enhanced Header with Download Button */}
              ...
              <View style={styles.headerRight}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (isExportingMemberStats || memberStats.length === 0) &&
                    styles.downloadButtonDisabled,
                  ]}
                  onPress={() => setShowMemberStatsExportMenu(true)}
                  disabled={isExportingMemberStats || memberStats.length === 0}
                >
                  <Download
                    size={20}
                    color={memberStats.length === 0 ? "#64748B" : "#6366F1"}
                    strokeWidth={2}
                  />
                </TouchableOpacity>
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                <View style={styles.tableContainer}>
                  <View style={styles.tableHeader}>
                    ...
                    <Text style={[styles.tableHeaderText, { width: 60 }]}>Meal</Text>
                    <Text style={[styles.tableHeaderText, { width: 90 }]}>Deposit</Text>
                    <Text style={[styles.tableHeaderText, { width: 90 }]}>Common</Text>
                    ...
                    <Text style={[styles.tableHeaderText, { width: 90 }]}>Balance</Text>
                  </View>
          ...
          {/* Member Stats Export Menu Modal */}
          <Modal
            visible={showMemberStatsExportMenu}
            transparent
            animationType="slide"
            onRequestClose={() => setShowMemberStatsExportMenu(false)}
          >
            <View style={styles.modalOverlay}>
              <TouchableOpacity
                style={styles.modalBackdrop}
                activeOpacity={1}
                onPress={() => setShowMemberStatsExportMenu(false)}
              />
              <View style={styles.exportModalCard}>
                <Text style={styles.modalTitle}>Export Report</Text>
                <Text style={styles.exportSubtitle}>
                  Choose format to download member statistics
                </Text>

                <TouchableOpacity
                  style={styles.exportOptionButton}
                  onPress={() => handleMemberStatsExport("pdf")}
                  disabled={isExportingMemberStats}
                >
                  {isExportingMemberStats ? (
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleMemberStatsExport("excel")}
                  disabled={isExportingMemberStats}
                >
                  {isExportingMemberStats ? (
                    <ActivityIndicator size="small" color="#FFF" />
