...
export default function MembersScreen() {
  ...
  // ── Animations ──
  const resultFadeIn = useRef(new Animated.Value(0)).current;
  const resultSlideUp = useRef(new Animated.Value(12)).current;
  const successScale = useRef(new Animated.Value(0)).current;

  // ── TOP-DOWN MODAL ANIMATIONS ──
  const modalSlideDown = useRef(new Animated.Value(-800)).current;
  ...
  useEffect(() => {
    const keyboardWillShow = Keyboard.addListener(
      ...
      (e) => {
        ...
        // Scroll to bottom when keyboard opens to show name input
        setTimeout(() => {
  ...
  // Animate modal in/out from TOP
  useEffect(() => {
    if (showAddModal) {
      // Reset values
      modalSlideDown.setValue(-800);
      ...
      Animated.parallel([
        ...
        Animated.spring(modalSlideDown, {
          toValue: 0,
          tension: 50,
          friction: 9,
          useNativeDriver: true,
      ...
      Animated.parallel([
        ...
        Animated.timing(modalSlideDown, {
          toValue: -800,
          duration: 250,
          useNativeDriver: true,
        }),
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── ADD MEMBER MODAL - TOP DOWN ── */}
      <Modal
        visible={showAddModal}
        transparent
        animationType="none"
        onRequestClose={() => { setShowAddModal(false); resetModalState(); }}
      ...
      >
        ...
        {/* Modal Card - Slides from TOP with Keyboard Avoidance */}
        <KeyboardAvoidingView
        ...
        >
          <Animated.View
            style={[
              styles.topModalCard,
              keyboardVisible && styles.topModalCardWithKeyboard,
              {
                transform: [
                  { translateY: modalSlideDown },
                  { scale: modalScale },
                ],
              },
            ]}
          >
            <ScrollView
              ...
              contentContainerStyle={styles.topModalScrollContent}
            >
              {/* Modal Header */}
                ...
                <View style={styles.newUserNameCard}>
                  ...
                  <View style={[styles.nameInputWrapper, nameError && styles.nameInputWrapperError]}>
                    <TextInput
                      ...
                      editable={!addingMember}
                      onFocus={() => {
                        // Scroll to end when name input is focused to ensure it's visible above keyboard
                        setTimeout(() => {
                          scrollViewRef.current?.scrollToEnd({ animated: true });
                        }, 300);
                      }}
...
const styles = StyleSheet.create({
  ...
  topModalCardWithKeyboard: {
    maxHeight: "75%",
  },
  topModalScrollContent: {
  ...
  modalIconBg: { width: 42, height: 42, borderRadius: 12, backgroundColor: "rgba(99,102,241,0.15)", alignItems: "center", justifyContent: "center" },
