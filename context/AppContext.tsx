...
export const AppProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    // Track active listeners to prevent duplicates and memory leaks
    ...
    /** Helper: returns true only for permission-denied errors during logout */
    const isLogoutPermissionError = (error: any): boolean => {
        if (!isLoggingOutRef.current && auth.currentUser) return false;
            ...
            membersUnsubscribeRef.current = onSnapshot(
                ...
                (error) => {
                    if (isLogoutPermissionError(error)) {
                        console.log("Members listener detached (user signed out)");
                        return;
                    }
    ...
    const initializeApp = useCallback(
        async (firebaseUser: FirebaseUser | null) => {
            ...
            try {
                if (!firebaseUser) {
                    // User not authenticated — immediately unsubscribe ALL listeners
                    ...
                    // Reset the flag after a tick so future errors aren't swallowed
                    setTimeout(() => { isLoggingOutRef.current = false; }, 500);
                    return;
                }

                // Authenticated — clear the logout flag
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(doc(db, "users", user.uid), (docSnap) => {
            // Guard: if user signed out while listener was in-flight, bail
            ...
            if (isLogoutPermissionError(error)) {
                console.log("User listener detached (user signed out)");
                return;
            }
